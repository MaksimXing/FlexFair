...
class FedTestMixed(object):
    ...
    def prediction(self):
        ...
        dict_A_CVC300 = {}
        dict_A_Kvasir = {}
        with open(self.args.datapath + 'Public_5/test/test_mean.csv', 'r') as file:
        ...
        with torch.no_grad():
            ...
            for image, mask, shape, name, origin, gt in self.loader:
                ...
                name_list.append(str(name[0][:-4]))
                
        dict_pred = {}
        for name, dice_value in zip(name_list, dice_list):
            dict_pred[name] = dice_value
        ...
        mean_dice_KVA = np.mean(pred_KVA)
        mean_dice_CVC = np.mean(pred_CVC)
        self.model.train(True)
        return mean_dice_KVA, mean_dice_CVC
...
def RunFedTestKVA_CVC(TrainingModel, args, cur_model_path, cur_epoch):
    _, mean_dice = RunFedTestMixed(args, TrainingModel, cur_model_path, cur_epoch)
    ret_0, ret_1 = calculateDP_PUBLIC(args, cur_model_path, cur_epoch)
    return ret_0, ret_1

    
def RunFedTestMixed_client(args, TrainingModel, cur_model_path, cur_epoch):
    writer = None
    t1 = FedTestMixed(FedTestDataKVA_CVC, TrainingModel, writer, args)
    mean_dice_KVA, mean_dice_CVC = t1.prediction()
    
    return mean_dice_KVA, mean_dice_CVC

def RunFedTestKVA_CVC_client(TrainingModel, args, cur_model_path, cur_epoch):
    mean_dice_KVA, mean_dice_CVC = RunFedTestMixed_client(args, TrainingModel, cur_model_path, cur_epoch)
