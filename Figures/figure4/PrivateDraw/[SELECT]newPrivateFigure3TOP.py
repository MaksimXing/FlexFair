...
paths_ = [f'your_data_path\\FedProx-miu\\seed0',
          ...
          f'your_data_path\\FairMixup\\seed0',
...
for seed in seeds:
    ...
    if os.path.exists(global_file):
        ...
        max_gap_each_row = np.maximum.reduce([gap_column_2, gap_column_3, gap_column_4, gap_column_5])  # get MaxGap
            ...
            for seed in seeds:
                ...
                if os.path.exists(global_file):
                    ...
                    max_gap_each_row = np.maximum.reduce(
                        [gap_column_2, gap_column_3, gap_column_4, gap_column_5])  # get MaxGap
...
colors = plt.cm.rainbow(np.linspace(0, 1, len(labels) + 2))  
...
for idx, label in enumerate(labels):
    ...
    if idx == 0:
        ...
        plt.plot(sorted_pareto_front[:, 0], sorted_pareto_front[:, 1], '^', markersize=8, color=colors[idx + 2], label=label)
    else:
        plt.plot(sorted_pareto_front[:, 0], sorted_pareto_front[:, 1], '-o', markerfacecolor='None', color=colors[idx + 2], label=label)
