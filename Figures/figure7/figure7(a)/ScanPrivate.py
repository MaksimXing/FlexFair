...
dir =
sites = ['zhongda', 'zhongzhong', 'new_zhongyi', 'shengfuyou']

smoothed_overall_histogram_list = []

for site in sites:
    image_dir = os.path.join(dir, site)
    ...
    original_images = [file for file in image_files if '_label' not in file]
    mask_images = [file for file in image_files if '_label' in file]
    ...
    for orig_file, mask_file in matched_images:
        ...
        mask_img = cv2.imread(os.path.join(image_dir, mask_file), 0)  # Assuming mask is also grayscale
...
with open('Private_histogram_data.pkl', 'wb') as f:
    pickle.dump({'smoothed_histograms': smoothed_overall_histogram_list}, f)

print("Data saved successfully to 'Private_histogram_data.pkl'.")
