    ...
    def __init__(self, DataList, FedAvgModel, args):
        ...
        for model_index in range(args.site_num):
            ...
            self.logger_list.append(
                SummaryWriter(args.savepath + '/FedAvg_OOD/' + self.local_time + '/log/Client' + str(model_index + 1)))
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            # for every epoch, every sub model run once
                ...
                for model_index in range(self.args.site_num):
                    ...
                    if model_index == 0:
                        for key in sub_net_para:
                            global_para[key] = sub_net_para[key] * 0.5
                    else:
                        for key in sub_net_para:
                            global_para[key] += sub_net_para[key] * 0.5
                # 更新全局模型
                ...
                cur_model_path = self.args.savepath + '/FedAvg_OOD/' + self.local_time + '/model/'
            ...
            for loader_model_index in range(self.args.site_num):
                if epoch + 1 > self.args.start_test_epoch:
                    # 这里乘2跳过重复计算CVC/KVA
                    cur_model_path = self.args.savepath + '/FedAvg_OOD/' + self.local_time + '/model/Client' + str(
                ...
                if epoch + 1 > self.args.start_save_model and (epoch + 1) % self.args.save_model_per_epoch == 0:
                    cur_path = self.args.savepath + '/FedAvg_OOD/' + self.local_time + '/model/Client' + str(
                        loader_model_index + 1) + '/'
        ...
        log_file = open(self.args.savepath + '/FedAvg_OOD/' + self.local_time + '/args.txt', "w")
