...
class FedProxTrainPublic(object):
    def __init__(self, DataList, FedProxModel, args):
        ...
        for model_index in range(args.site_num):
            ...
            self.logger_list.append(
                SummaryWriter(args.savepath + '/FedProx/' + self.local_time + '/log/Client' + str(model_index + 1)))
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            for loader_model_index in range(self.args.site_num):
                ...
                if epoch + 1 > self.args.start_test_epoch:
                    cur_model_path = self.args.savepath + '/FedProx/' + self.local_time + '/model/Client' + str(
                ...
                if epoch + 1 > self.args.start_save_model and (epoch + 1) % self.args.save_model_per_epoch == 0:
                    cur_path = self.args.savepath + '/FedProx/' + self.local_time + '/model/Client' + str(
                                       loader_model_index + 1) + '/'
                    if not os.path.exists(cur_path):
                        os.makedirs(cur_path)
                    torch.save(self.sub_model_list[loader_model_index].state_dict(),
                               self.args.savepath + '/FedProx/' + self.local_time + '/model/Client' + str(
        ...
        log_file = open(self.args.savepath + '/FedProx/' + self.local_time + '/args.txt', "w")
