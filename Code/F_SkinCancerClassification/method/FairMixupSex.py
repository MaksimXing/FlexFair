...
def FairMixupSexTrain(args, kwargs):
    com_counter = 0

    now = datetime.now()
    local_time = now.strftime("%m%d-%H%M%S")
    path = f'output/sex-mixup/s{args.seed}_w{args.penaly_weight}/' + local_time
    ...
    model_state_dict = torch.load(f'../models/seed{args.seed}_model.pth')
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        while True:
            ...
            for loader_model_index in range(2):
                ...
                # mixup
                sex = sexs[loader_model_index]
                zero_sex_indices = (sex == 0).nonzero(as_tuple = True)[0]
                one_sex_indices = (sex == 1).nonzero(as_tuple = True)[0]
                
                batch_x1 = img_1[zero_sex_indices]
                batch_x2 = img_1[one_sex_indices]
