from Code.F_SkinCancerClassification.util import HAM_data_preparation, BCN_data_preparation, mkdiry, FedTest
# import torchvision.models as models
from Code.F_SkinCancerClassification.method import resnet
...
def FedAvgTrain(args, kwargs):
    com_counter = 0

    now = datetime.now()
    local_time = now.strftime("%m%d-%H%M%S")
    path = f'output-{args.compute_type}/FedAvg/s{args.seed}_w{args.penalty_weight}/' + local_time
