...
class NewFedAvgTrainPrivate(object):
            ...
            # for every epoch, every sub model run once
                ...
                for loader_index in range(self.args.site_num):
                    ...
                    self.logger_list[loader_index].add_scalar('communication/global_model_total_size)',
                                                              global_net_size,
                                                              epoch)
                cur_model_path = self.args.savepath + '/FedAvg/' + self.local_time + '/model/'
                ...
                ret_0, ret_1, ret_2, ret_3 = RunFedTestPrivateBaseline(self.global_model,
                                                 self.args, cur_model_path, epoch)
                f = open(cur_model_path + "global.csv", 'a+')
                writer = csv.writer(f)
                writer.writerow([str(ret_0[0]), str(ret_0[1]), str(ret_0[2]), str(ret_0[3]),
                                 str(ret_0[4]), str(ret_0[5]), str(ret_0[6]), str(ret_2[5]), str(ret_2[6])])
                f.close()
