...
class FedAvgModelForMixup(nn.Module):
    def __init__(self, args):
        super(FedAvgModelForMixup, self).__init__()
        self.args    = args
        channels = []
    ...
    def forward(self, x, shape=None):
        ...
        pred_1 = torch.cat([out5, out4*out5, out3*out4*out5], dim=1)
        pred = self.predict(pred_1)
        return pred, pred_1

    def predict_feature(self, pred_1):
        pred = self.predict(pred_1)
        return pred
