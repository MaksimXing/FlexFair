...
def runDraw(args, base_path, image_name, label_name, model_path_1, model_path_2, output_pic_path):

    info_1 = generate_segmentation_and_gradcam(args, model_path_1, os.path.join(base_path, image_name))
    info_2 = generate_segmentation_and_gradcam(args, model_path_2, os.path.join(base_path, image_name))

    # Load and process the ground truth mask
    gt_mask_path = os.path.join(args.test_data_path, base_path, label_name)  # Adjust if mask has a different naming convention
